...
// Targets AL (67 counties) + GA (20 counties) via JS-rendered pages
...
// ── Priority seed list: AL + GA counties ──
...
// ── Parse property rows from GovEase DOM ────────────────────
// Runs inside page.evaluate — browser context only
function extractPropertiesFromDOM() {
  ...
  rows.forEach(function(row) {
    ...
    // Need at least a parcel-like ID
  ...
  // Strategy 2: GovEase auction cards / listing blocks
  if (results.length === 0) {
  ...
  // Strategy 3: GovEase main auction listing page
  // URL: /tax-sale-property-auctions — lists active auctions with property counts
  if (results.length === 0) {
    var listItems = document.querySelectorAll('.list-group-item, .sale-listing, [class*="event-row"]');
    listItems.forEach(function(item) {
      var text = (item.textContent || '').replace(/\s+/g, ' ').trim();
      var idMatch = text.match(/\b([A-Z0-9\-]{5,})\b/);
      if (!idMatch) return;
      var addrMatch = text.match(/\d+\s+[A-Z][a-z]+\s+(?:St|Ave|Rd|Dr|Ln|Blvd|Ct|Way|Pl)[,.\s]/i);
